...
| `id_ed25519` / `id_rsa` | Закрытый SSH-ключ (создаётся при генерации). **Скачайте и удалите с сервера!** |
...
2. Ключи генерируются без passphrase:
   - Открытый ключ → `.token`
   - Закрытый ключ → `id_ed25519` или `id_rsa` (рядом со скриптом)

3. **⚠ Скрипт покажет предупреждение** — обязательно скачайте закрытый ключ до продолжения, иначе потеряете доступ к серверу!

### Подключение с новым ключом

```bash
ssh -i /путь/к/id_ed25519 -p 2288 root@<IP-сервера>
```
